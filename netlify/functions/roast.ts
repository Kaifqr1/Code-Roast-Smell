/**
 * Netlify Function — POST /api/roast (redirected from netlify.toml)
 * Set GROQ_API_KEY in your Netlify site environment variables.
 */
...
export const handler = async (event: any) => {
  ...
  const parsed = RoastCodeBody.safeParse(body);
  if (!parsed.success) {
  ...
  if (!apiKey) {
    return {
      ...
      body: JSON.stringify({
        error:
          "Groq API key not configured. Add GROQ_API_KEY to your Netlify environment variables. Get a free key at console.groq.com.",
      }),
    };
  }
  ...
  try {
    ...
    if (!result.success) {
      return { statusCode: 500, body: JSON.stringify({ error: "LLM response did not match expected shape." }) };
    }
    ...
    if (status === 429) {
      return { statusCode: 429, body: JSON.stringify({ error: "Groq rate limit reached. Wait a moment and try again." }) };
    }
    if (status === 401) {
      return { statusCode: 401, body: JSON.stringify({ error: "Invalid Groq API key. Check your GROQ_API_KEY environment variable." }) };
    }
    return { statusCode: 502, body: JSON.stringify({ error: "Groq API returned an error. Try again shortly." }) };
  }
};
